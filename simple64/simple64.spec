...
sed \
  -e 's|"../../../mupen64plus-core/src/api"|%{_includedir}/mupen64plus|g' \
  -i %{name}-{audio-sdl2,input-raphnetraw}/projects/unix/Makefile
...
%else
sed \
  -e 's|"include/mupen64plus"|include/%{name}|g' \
  -i %{name}-{audio-sdl2,input-raphnetraw}/projects/unix/Makefile

sed -e 's|libmupen64plus|lib%{name}|g' -i mupen64plus-core/projects/unix/Makefile
%endif
...
%if %{without mupen64plus}
%make_build V=1 -C mupen64plus-core/projects/unix all
%endif

for i in %{name}-input-qt %{name}-gui ;do
  mkdir $i/build
  pushd $i/build
    %{qmake_qt6} ../$i.pro
  popd
...
for i in %{name}-audio-sdl2 %{name}-input-raphnetraw ;do
  pushd $i
    %make_build V=1 -C projects/unix all
  popd
done

for i in parallel-{rdp-standalone,rsp} ;do
pushd $i
%cmake \
...
install -pm0755 %{name}-gui/build/%{name}-gui %{buildroot}%{_bindir}/

mkdir -p %{buildroot}%{_libdir}/%{name}

%if %{without mupen64plus}
cp -a mupen64plus-core/projects/unix/lib%{name}.so.* %{buildroot}%{_libdir}/
...
install -pm0755 %{name}-input-qt/build/lib%{name}-input-qt.so \
  %{buildroot}%{_libdir}/%{name}/%{name}-input-qt.so
...
install -pm 0644 %{name}.appdata.xml %{buildroot}%{_metainfodir}/%{name}.appdata.xml
