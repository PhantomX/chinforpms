...
sed \
  ...
  -e '/^Libs.private:/d' \
  -e "s,@PROJECT_VERSION@,$(awk '/BELR VERSION/{print $3}' CMakeLists.txt),g" \
  %{name}.pc.in > %{name}.pc
...
%{_includedir}/%{name}/*.hh
%{_includedir}/%{name}/*.cc
%{_datadir}/Belr/cmake/*.cmake


%changelog
* Fri Sep 22 2017 Phantom X <megaphantomx at bol dot com dot br> - 0.1.3-1
- Initial spec
