...
** "wants"   - (default) rule is satisfied if kmod ends up in listed package
              (or any package it depends on - meaning it can move
              down in package hierarchy)
** "needs"   - enabled by setting `exact_pkg: True` on the rule.
              Hard-locks matched kmods to exactly one package.
              The kmod cannot be moved to satisfy other constraints.
...
==== Rule attributes
Rules support optional attributes:

* `exact_pkg: True` - promotes the rule from "wants" to "needs",
  hard-locking matched kmods to the specified package.
* `ignore_deps: True` - severs all dependency edges of matched kmods
  so they do not constrain (or get constrained by) their dependencies.
  Useful for test/kunit modules that depend on production kmods but
  should be placed in a sibling package (e.g. modules-internal)
  without dragging their dependencies out of intended packages.
  Only meaningful together with `exact_pkg: True`.
* `if_variant_in: [list]` - makes the rule conditional on variant.
...
== How does it work?
It uses constraint propagation (arc consistency). Each kmod maintains
a set of allowed packages -- candidates that won't break any rules.
Initially every kmod is allowed in all packages. Yaml config rules
narrow these sets, and then a worklist-based propagation prunes
infeasible packages by checking kmod dependency neighbors.

Each package precomputes two reachability sets:
`self_and_below` (self + all packages it transitively depends on) and
`self_and_above` (self + all packages that transitively depend on it).
These make constraint checks simple set intersections.

.There are 4 phases:
1. *Apply initial labels* from yaml config:
   "needs" rules (exact_pkg) lock a kmod to exactly one package
   (and sever its dependency edges if ignore_deps is set),
   "wants" rules restrict a kmod to the target package and its ancestors,
   "default" marks the fallback package.
2. *Propagate constraints* (worklist-based): for each kmod, remove
   packages from its allowed set if no compatible package exists in
   a dependency neighbor's allowed set. When a kmod's allowed set
   shrinks, its neighbors are re-queued for checking.
3. *Resolve preferred*: for kmods still with multiple candidates,
   pick the one matching the "wants" rule preference (or nearest ancestor
   in the package hierarchy).
4. *Resolve remaining*: for any kmods still ambiguous, pick the default
   package (or nearest ancestor), processing in reverse topological order.
